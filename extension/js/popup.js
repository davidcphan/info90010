window.onload=function(){
    ...
    document.getElementById('space').addEventListener('click',function(){
        chrome.tabs.create({ url: "http://fedora.studymates/my-groups/" });
    });
};
...
function getTasks(){
    ...
    xhr.open('POST', 'http://fedora.studymates/wp-json/api/v1/tasks', true);
...
function removeTask(x){
    //Send AJAX request to retrieve task list
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'http://fedora.studymates/wp-json/api/v1/remove-task', true);
    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhr.onload = function () {
...
function completeTask(x){
    //Send AJAX request to retrieve task list
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'http://fedora.studymates/wp-json/api/v1/complete-task', true);
    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhr.onload = function () {
        window.location.reload();
    };
